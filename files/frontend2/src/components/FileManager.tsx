      ...
      {fileStatuses.length === 0 ? (
        ...
        <Row gutter={[24, 24]}>
          {fileStatuses.map((fileStatus) => (
            <Col xs={24} sm={12} lg={8} key={fileStatus.file.file_id}>
              <Card
                hoverable
                className="h-full"
                title={
                ...
                extra={getStatusBadge(fileStatus.status, fileStatus.fileInfo)}
                actions={[
                  <Button
                    key="toggle"
                    type="text"
              ...
              >
                <Descriptions column={1} size="small">
                  <Descriptions.Item label="Rows">
                    {fileStatus.file.row_count.toLocaleString()}
                  </Descriptions.Item>
                  <Descriptions.Item label="Columns">
                    <Space>
                      <span>{fileStatus.file.columns.names.length} total</span>
                      <Tag color="green" size="small">
                  ...
                  {fileStatus.fileInfo?.has_visualizations && (
                    <Descriptions.Item label="Visualizations">
                      <Space>
                        <span>{fileStatus.fileInfo.visualization_count}</span>
                        {fileStatus.fileInfo.available_methods.map(method => (
                          <Tag key={method} color="purple" size="small">{method.toUpperCase()}</Tag>
                        ))}
                      </Space>
                    </Descriptions.Item>
                  )}
                  <Descriptions.Item label="Uploaded">
                    {new Date(fileStatus.file.created_at).toLocaleDateString()}
                  </Descriptions.Item>
                </Descriptions>
